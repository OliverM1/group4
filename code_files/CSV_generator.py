...
def invalid_entry_csv(year, month, day, start, num):
    """Creates 'num' amount of csv files with an invalid entry"""
    ...
    for i in range(start, num + start):
        with open("MED_DATA_" + df.year_format(year,month,day) + df.time_format(i) + ".csv", "w", newline="") as csvfile:
            ...
            for n in range(0, 10):
                if error_column == 1:
                    row = [ran.randint(1, 199), "00:00:70"]
                else:
                    row = [ran.randint(1, 199), "00:00:00"]
                    for m in range(0, 10):
                        row.append(round(ran.random() * 10, 3))
...
def csv_generator(*, year = "2021", month = "1", day = "1", empty = 0, correct = 0, missing_header = 0, missing_column = 0, invalid_entries = 0, duplicate_batch_ids = 0, incorrect_filename = 0):
    """"Takes the date and the number of each type of file you'd like to generate and creates them,
    naming starts at MED_DATA_YYYYMMDD000000.csv and goes up in seconds (where YYYYMMDD corresponds to the date you entered)."""

    # This limits the number of files that can be created at once adn returns an error message if the user asks for too many
    MAX_LIMIT = 100
    ...
    # Each type of file is generated in order, then the total is added to to make sure the same filename isn't used twice. Note: The file names will be sequential
    total = 0
...
#Asks the user for the date they'd like to generate files for and the type of each file they'd like, then runs the function to create those files
...
csv_generator(year = year, month = month, day = day, correct = correct, empty = empty, missing_header = missing_header, missing_column = missing_column, duplicate_batch_ids = duplicate_batch_id, invalid_entries = invalid_entry, incorrect_filename = incorrect_filename)
